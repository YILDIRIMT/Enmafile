...
void controllers_set_true() {
	var_struct_ref_parser.sys_controller    = true;
	var_struct_ref_parser.target_controller = true;
}

std::string parsers::line_parser(const std::string& line, std::string pars_symbol_start, std::string pars_sybol_end, int to_parse) {
    ...
    while(pos != std::string::npos) {
        ...
        if(to_parse == 0) {
            ...
            if (tfi == -1) {return ERROR_VAR_NOT_EXISTS + temp_parameter;}
            else {temp += var_struct_ref_parser.var_value[tfi];}
        } else {temp += run::execute_command_and_return(temp_parameter.c_str());} // to_parse != 0 -> system command parsing
...
bool parsers::var_def_parser(std::string read_line, int line_counter) {
	if(read_line.substr(0,1) == VAR_KEY) {
		if(run::error_control(enma_utils::find_var_name(read_line.substr(1,read_line.length()), ASSIGNMENT_KEY), line_counter)) {return false;}
		else if(run::error_control(enma_utils::find_var_value(read_line, ASSIGNMENT_KEY), line_counter)) {return false;}
		else {
			...
			if(tfi != -1) {var_struct_ref_parser.var_value[tfi] = temp_var_value;}
			else {
				//create new var
...
int parsers::detect_condition_parser(std::string& condition_data, std::string& read_line, int key_control, int key_length, bool eq_state) {
	///* check condition (if) or (ifeq) */
	if(key_control != -1) {
		// get condition data
		...
		bool ref_ccparser = parsers::condition_control_parser(str_utils::ltrim(condition_data_ltrimmed.substr(def_key_ref.length(), condition_data_ltrimmed.length() - def_key_ref.length())), eq_state, 2);
				
		if(condition_data_ltrimmed.substr(0, def_key_ref.length()) == DEF_KEY && ref_ccparser) {return 1;}
		else if(condition_data_ltrimmed.substr(0, def_key_ref.length()) == DEF_KEY && !ref_ccparser) {return 0;}
	
		// condition check for match *trigger
		if (condition_control_parser(condition_data, eq_state, 1)) {return 1;}
		else {return 0;}
	} return -1;
}

bool parsers::condition_control_parser(const std::string& data, bool eq, int toControl) {
	if(toControl == 1) {
		...
		if(find_index_ref != -1 && !eq) {return true;}
		else if(find_index_ref == -1 && eq) {return true;}
		else {return false;}
	} return false;
}

bool parsers::handle_condition_parser(bool condition_controller, std::string& read_line, int line_counter) {
	if(condition_controller == true) {
		if(read_line.substr(0,1) != AC_CON_KEY) {return false;}
		else {var_def_parser(str_utils::ltrim(read_line), line_counter);} 
    	return true;
    } return false;
}

int parsers::deps_parser(const std::string& target, const std::string& deps) {
	if(!std::filesystem::exists(target)) {return -1;}
	else {
		...
		while(isdata >> deps_temp) {
			if(!std::filesystem::exists(deps_temp)) {return -2;}
			// if deps newer then target 
			if(target_time < std::filesystem::last_write_time(deps_temp)) {return 0;}
		} return -3;
	}
}

void parsers::main_parser(const std::string& file_path, const std::string& argument) {
	...
	while(std::getline(enmfilef, read_line)) {
		...
		read_line = parsers::line_parser(read_line, USE_VAR_START, USE_VAR_END, 0);
		...
		if(read_line.substr(0, var_not_exists_error.length()) == ERROR_VAR_NOT_EXISTS) {
			...
			break;
		} else {read_line = parsers::line_parser(read_line, "s>", "<s", 1);}
		
		// === comment line control ===
		if(read_line.substr(0,1) == COMMENT_KEY) {continue;}
	    ...
	    if(!handle_condition_parser_result) {condition_controller = handle_condition_parser_result;}
		else {continue;}
		
		// === control for "ifeq" condition ===
		key_control = str_utils::find_key_index(read_line, IFEQ_KEY);
		condition_key_for_length = IFEQ_KEY;                                                                                                //eq
		...
		if(directive_controller == true) {
			...
			var_struct_ref_parser.target_controller = true;
			if(read_line.substr(0,1) != DIRECTIVE_KEY) {directive_controller = false;}
			else {run::execute_directive(read_line, silence_control);}
		} if(str_utils::trim(read_line) == argument) {directive_controller = true;}
		
		// === register variables on memory(vectors) ===
		if(!var_def_parser(read_line,line_counter)) {break;}
		
	} if(var_struct_ref_parser.target_controller == false) {run::error_control(ERROR_TARGET_NOT_EXISTS, 0);}
}
