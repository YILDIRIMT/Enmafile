...
namespace str_utils {
	...
	std::string trim(const std::string& data) {return rtrim(ltrim(data));}
	...
	int find_index(const std::vector<std::string>& find_index_vector, const std::string& find_index_value) {
		for(int counter = 0; counter != find_index_vector.size(); counter++) {
			if(find_index_vector[counter] == find_index_value) {return counter;}
		} return -1;
	}
}
...
namespace enma_utils {
	std::string find_var_name(const std::string& line, char assignment_char) {
		if(line.length() < 2) {return ERROR_NULL_VAR_NAME;}
		...
		if(pos < 2) {return ERROR_NULL_VAR_NAME;}
	...
	std::string find_var_value(const std::string& line, char assignment_char) {
		...
		if(pos == std::string::npos) {return ERROR_NULL_VALUE;}
		if(str_utils::ltrim(line.substr(pos + 1, line.length())).empty()) {return ERROR_NULL_VALUE;}
